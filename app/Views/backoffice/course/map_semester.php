					...
					<div class="m-portlet__head-tools">
						<ul class="m-portlet__nav">
							<li class="m-portlet__nav-item">
								...
								<a href="<?php echo ($act) ? $act : '' ?>" style="<?php echo $show;?>" class="btn m-btn--pill btn-outline-success m-btn m-btn--custom m-btn--icon ">
					...
					<table class="m-datatable" id="html_table" width="100%">
						<thead>
							<tr>
								...
								<th>
									<center>Semester Status</center>
								</th>
								<th>
									<center>Display Order</center>
								</th>
								<th>
									<center>Active / Inactive</center>
								</th>
						...
						<tbody>
							<?php
								if(!empty($semAllData)){
									foreach($semAllData as  $key => $value){
										$course_id	=	$value['course_id'];
										
							?>
							<tr>
								<td><center><?php echo ++$key;?></center></td>
								<td><center><?php echo $value['semester'].'  semester';?></center></td>
								...
								<td>
									<center>
											<div class="m-btn-group m-btn-group--pill btn-group" role="group" aria-label="First group">
												...
												<a href="<?php echo base_url('backoffice/course/sem_edit/'.($course_id.'/'.$value['id']))?>">
														<span class="btn m-btn--square  m-btn m-btn--gradient-from-info m-btn--gradient-to-accent"><i class="fa fa-pencil-alt"></i></span>
													</a>
												
												
													<?php if($value['status']==1){ ?>
													<a onclick="return confirm('Do you sure want to deactive this record.');" href="<?php echo base_url(); ?>backoffice/course/sem_deactive/<?php echo ($course_id.'/'.$value['id']); ?>" title="Deactive" > 
													...
													<a>	
													<?php } else { ?>
													<a onclick="return confirm('Do you sure want to active this record.');" href="<?php echo base_url(); ?>backoffice/course/sem_active/<?php echo ($course_id.'/'.$value['id']); ?>" title="Active" >
